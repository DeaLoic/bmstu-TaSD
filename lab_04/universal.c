...
int is_date_format(char *date_str)
{
    ...
    if (date_str && strlen(date_str) == DATE_LEN_DOTS - 1)
    {
        if (date_str[2] == date_str[5] && date_str[2] == '.' && isdigit(date_str[0]) &&
            isdigit(date_str[1]) && isdigit(date_str[3]) && isdigit(date_str[4]) &&
            isdigit(date_str[6]) && isdigit(date_str[7]) && isdigit(date_str[8]) &&
            isdigit(date_str[9]))
        {
            exit_code = 1;
        }
